...
const OurProvider: React.FC<{ ourProviderRef: React.RefObject<IHandles | null>, service: BusinessType }> = ({ ourProviderRef, service }) => {
    const [message, setMessage] = useState("")
    const [load, setLoad] = useState(false)

    const sendServiceLead = async () => {
        setLoad(true)
        const response = await sendLead({provider: service?.provider?._id!, message: message})
        ...
        >
            {
                true ?
                    <View style={{...styles.modalContent, height: 600}}>
                        <Text style={{ marginTop: 6, fontSize: 18, fontWeight: 600, textAlign: "center" }}>Send a message to {service?.title}</Text>
                    ...
                    <View style={styles.modalContent}>
                        ...
                        <Pressable onPress={()=> router.push(`/chat/${service?.provider?._id}/null`)} style={styles.bookButton}><Text style={{ color: "white", fontSize: 16 }}>Chat Service</Text></Pressable>
                        <Pressable onPress={()=> makeCall(service?.provider?.phoneNumber!)} style={styles.bookButton}><Text style={{ color: "white", fontSize: 16 }}>Call Service</Text></Pressable>
                    </View>
            }
        </Modalize>
    )
